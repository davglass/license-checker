...
var flatten = function(options) {
    ...
    if (include(options.customFormat.repository) && json.repository) {
    ...
    if (include(options.customFormat.url) && json.url) {
        /*istanbul ignore next*/
    ...
    if (json.author && typeof json.author === 'object') {
        ...
        if (include(options.customFormat.publisher) && json.author.name) {
            moduleInfo.publisher = json.author.name;
        }
        if (include(options.customFormat.email) && json.author.email) {
            moduleInfo.email = json.author.email;
        }
        if (include(options.customFormat.url) && json.author.url) {
    ...
    files.forEach(function(filename) {
        ...
        if (fs.lstatSync(licenseFile).isFile()) {
			...
			if (include(options.customFormat.licenseFile)) {
				moduleInfo.licenseFile = options.basePath ? path.relative(options.basePath, licenseFile) : licenseFile;
			}
			
            if (include(options.customFormat.licenseText) && options.customFormat) {
